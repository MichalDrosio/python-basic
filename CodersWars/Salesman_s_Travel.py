...
code = ("OH 43071,NY 56432,ZP 32908,AE 56210,RE 13000,EX 34342,SW 43098,AA 45521,GG 30654,ZP 32908,AE 56215,RE 13200,"
        "EX 34345, RE 13222,RE 13001,SW 43198,AA 45522,GG 30655,XX 32321,YY 45098")
...
def travel(r, zipcode):
    ...
    for address in r:
        if zipcode in address:
            adre = address
            adre = adre[:len(adre)-len(zipcode)]
            street = adre.lstrip('1234567890').strip()
            numbres = adre[:len(adre)-(len(street)+2)]
            x.append(numbres)
            y.append(street)
    return f"{zipcode}:{','.join(y)}/{','.join(x)}"
