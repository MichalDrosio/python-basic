...
#
# For example, to type a letter "R" you would press the 7 key three times (as the screen display for
# the current character cycles through P->Q->R->S->7). A character is "locked in" once the user presses a
# different key or pauses for a short period of time (thus, no extra button presses are required beyond what is
# needed for each letter individually). The zero key handles spaces, with one press of the key producing a
...
def presses(phrase):
    ...
    for letter in phrase.lower():
        press += get_presses(c=letter)
    return press


print(presses('michal'))
