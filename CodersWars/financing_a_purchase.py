...
#
# We have: c = n /d with n = r * balance and d = 1 - (1 + r)**(-term) where ** is the power function
# (you can look at the reference below).
#
# The payment c is composed of two parts. The first part pays the interest (let us call it int)
# due for the balance of the given month, the second part repays the balance (let us call this part princ)
# hence for the following month we get a new balance = old balance - princ with c = int + princ.
#
# Loans are structured so that the amount of principal returned to the borrower starts out small and
# increases with each mortgage payment. While the mortgage payments in the first years consist primarily
# of interest payments, the payments in the final years consist primarily of principal repayment.
#
# A mortgage's amortization schedule provides a detailed look at precisely what portion of each mortgage payment
# is dedicated to each component.
#
# In an example of a $100,000, 30-year mortgage with a rate of 6 percents the amortization schedule consists of
# 360 monthly payments. The partial amortization schedule below shows with 2 decimal floats
...
def amort(rate, bal, term, num_payments):
    ...
    for i in range(num_payments):
        ...
        princ = c - int_
        bal -= princ
    return f"num_payment {num_payments} c {round(c)} princ {round(princ)} int {round(int_)} balance{round(bal)}"
