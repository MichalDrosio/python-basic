...
# Each index travelled requires one unit of momentum.
# (The subject starts outside of the array, so it requires 1 momentum to get to index 0).
#
...
# If the subject does make it to the last index of the array, with at least 1 momentum remaining
# (required to leave the array), it has successfully reached the "other side".
#
# Examples:
#
...
def survivors(list_of_momentum, list_of_powerups):
    result = []
    for i, (mntm, pwups) in enumerate(zip(list_of_momentum, list_of_powerups)):
        for pwup in pwups:
            mntm -= 1
            if mntm < 0:
                break
...
print(survivors([3, 2, 1, 0], [[1, 0, 0], [0, 2, 0, 0], [0, 9], [8, 8]]))