# Task
# In 10 years of studying, Alex has received a lot of diplomas.
# He wants to hang them on his wall,
# but unfortunately it's made of stone and it would be too difficult to hammer so many nails in it.
#
# Alex decided to buy a square desk to hang on the wall, on which he is going to hang his diplomas.
# Your task is to find the smallest possible size of the desk he needs.
#
...
def diplomas(h, w, n):
    size = int((h * w * n) ** .5)
    print(size)
    while (size // h) * (size // w) < n:
        print(f'{size//h} * {size//w} == {(size//h)*(size//w)}')
        size += 1
    return size


print(diplomas(2, 3, 10))
