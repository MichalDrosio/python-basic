...
# remove this digit at that index and insert it back to another or at the same place in
# the number in order to find the smallest number you can get.
#
# Task:
...
def smallest(n):
    ...
    for i in range(len(str(n))):
        cut = str(n)[:i] + str(n)[i+1:]
        for j in range(len(cut)+1):
            new = int(cut[:j] + str(n)[i] + cut[j:])
            if new < array[0]:
                array = [new,i,j]
    return array
