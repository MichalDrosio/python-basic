...
# I don't know if my approach was good or bad, but in one of these steps
# I was writing a function to convert word to numbers. I did it.. eventually, but...
# I didn't like how it was written. So I thought why not create kata and check how other people do it :) So:
#
# Task:
...
def convert(st):
    ...
    for letter in st:
        if letter in letters.keys():
            ans += str(letters[letter])
        else:
            ans += str(nums[len(letters)])
            letters[str(letter)] = nums[len(letters)]
