...
# but lexicographically, as in a dictionary (first by the first digit, then, if the first digit is equal, by the second,
# and so on). In addition, we do not consider an infinite set of natural numbers, but only the first n numbers.
#
...
def strange_math(n, k):
    temp = sorted([str(i) for i in range(1, n + 1)])
    temp = [int(i) for i in temp]
    return len(temp[:temp.index(k) + 1])
